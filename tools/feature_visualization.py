...
def featuremap_process(feature_map):
    ...
    return feature_map
def featuremap_process_se(feature_map):
    assert isinstance(feature_map, torch.Tensor)
    feature_map = feature_map.detach()
    feature_map = feature_map.cpu().numpy()
    ...
    # feature_map = feature_map/np.sum(feature_map)

    return feature_map
#专门给se_b的
def featuremap_process_se1(feature_map):
    assert isinstance(feature_map, torch.Tensor)
    feature_map = feature_map.detach()
    feature_map = feature_map.cpu().numpy()
    return feature_map
...
def draw_feature_map_channel(features,save_image_path,save_dir = 'conv_ht',name = None):
    ...
    if isinstance(features,torch.Tensor):
        ...
        for onebat_features in features:
            ...
            # onebat_features = featuremap_process(onebat_features)
            ...
            for i,onebat_onechan_features in enumerate(onebat_features):
                ...
                onebat_onechan_features=cv2.applyColorMap(onebat_onechan_features, cv2.COLORMAP_JET)

                b, g, r = cv2.split(onebat_onechan_features)
                onebat_onechan_features = cv2.merge((r, g, b))
...
def draw_feature_map_channel_compare_se(features,save_image_path,save_dir = 'conv_ht',name = None):

    if isinstance(features,torch.Tensor):
        plt.clf()
        # features=featuremap_process_se(features)
        for onebat_features in features:
            features_channel=onebat_features.shape[0]
            features_channel_sqrt=math.ceil(features_channel ** 0.5)
            ...
            for i,onebat_onechan_features in enumerate(onebat_features):
                ...
                plt.axis('off')
                # plt.imshow(onebat_onechan_features,cmap='gray')
                plt.imshow(onebat_onechan_features)
                save_path_ele = save_path + '/' + str(i)+'.jpg'
                mmcv.imwrite(onebat_onechan_features,save_path_ele)
            ...
            if isinstance(features, torch.Tensor):
                ...
                for onebat_features in features:
                    ...
                    for i, onebat_onechan_features in enumerate(onebat_features):
                        ...
                        b, g, r = cv2.split(features1)
                        features1 = cv2.merge((r, g, b))
                ...
                print('ht_error')
def draw_feature_map_channel_se1(features,save_image_path,save_dir = 'conv_ht',name = None):

    if isinstance(features,torch.Tensor):
        plt.clf()
        save_path = os.path.join(save_image_path, save_dir)
        creat_dir(save_path)
        features1 = featuremap_process(features)
        features1 = np.uint8(255 * features1)
        features1=cv2.applyColorMap(features1, cv2.COLORMAP_JET)
        ...
        save_path_ele = save_path + '/' + str(1)+'.jpg'
        mmcv.imwrite(features1,save_path_ele)
        # plt.figure(figsize=(features_channel_sqrt*10, features_channel_sqrt*10))
        save_path_sum=save_path+'.jpg'
        plt.savefig(save_path_sum)
    else:
...
def draw_feature_map_channel_se2(features,save_image_path,save_dir = 'conv_ht',name = None):
    if isinstance(features,torch.Tensor):
        plt.clf()
        save_path = os.path.join(save_image_path, save_dir)
        creat_dir(save_path)
        features=torch.squeeze(torch.squeeze(features,2),2)
        features1 = featuremap_process_se(features)
        features1 = np.uint8(255 * features1)
        features1 = cv2.applyColorMap(features1, cv2.COLORMAP_JET)
    ...
    #         creat_dir(save_path)
    #         onebat_features=featuremap_process(onebat_features)
    #         onebat_features=np.uint8(255 * onebat_features)
    #
    #
    #         for i,onebat_onechan_features in enumerate(onebat_features):
    #             # onebat_onechan_features=featuremap_process(onebat_onechan_features)
    ...
    #     print('ht_error')
# 专门给se_b的
def draw_feature_map_channel_se3(features,save_image_path,save_dir = 'conv_ht',name = None):
    if isinstance(features,torch.Tensor):
        plt.clf()
        save_path = os.path.join(save_image_path, save_dir)
        creat_dir(save_path)
        features=torch.squeeze(torch.squeeze(features,2),2)
        features1 = featuremap_process_se1(features)
        features1 = np.uint8(255 * features1)
        features1 = cv2.applyColorMap(features1, cv2.COLORMAP_JET)
