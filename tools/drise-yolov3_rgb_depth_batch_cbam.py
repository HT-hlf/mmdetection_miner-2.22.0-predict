...
def mask_image(image, mask):
    masked = ((image.astype(np.float32) / 255 * np.dstack([mask] * 4)) *
              255).astype(np.uint8)
    return masked
...
def main(args):
    ...
    rgb_path = r"G:\mmdetection_miner\data\ht_cumt_rgbd_drise\val2014/"
    depth_path = r"G:\mmdetection_miner\data\ht_cumt_rgbd_drise\depth_val/"
    ...
    for i, f in enumerate(rgb_list):
        ...
        if res[0][0].shape[0] != 0:
            ...
            image_with_bbox_depth, heatmap = gen_cam(depth_data, saliency_map)
            # cv2.rectangle(image_with_bbox, tuple(target_box[:2]), tuple(target_box[2:]),
            #           (0, 0, 255), 5)
            # cv2.rectangle(image_with_bbox_depth, tuple(target_box[:2]), tuple(target_box[2:]),
            #               (0, 0, 255), 5)

            # mkdir(args.save_dir)
            # save_path = os.path.join(args.save_dir, 'rgb',f.split(".")[0] + ".jpg")
            # save_path_depth = os.path.join(args.save_dir,'depth',
            #                                f.split(".")[0] + '_depth' + ".jpg")
            # cv2.imwrite(save_path, image_with_bbox)
            # cv2.imwrite(save_path_depth, image_with_bbox_depth)

            rgb_path_ht = os.path.join(args.save_dir, 'rgb', )
            mkdir(rgb_path)
            depth_path_ht = os.path.join(args.save_dir, 'depth', )
            mkdir(depth_path)

            # mkdir(args.save_dir)
            save_path = os.path.join(rgb_path_ht, f.split(".")[0] + ".jpg")
            save_path_depth = os.path.join(depth_path_ht,
                                           f.split(".")[0] + ".jpg")
            cv2.imwrite(save_path, image_with_bbox)
            cv2.imwrite(save_path_depth, image_with_bbox_depth)
...
def parse_args():
    ...
    parser.add_argument('--config',
                        type=str,
                        default = 'configs/yolo/yolov3_d53_mstrain-416_273e_coco_rgb_depth_attention_cbam_d.py',
                        help='Yolo V3 configuration.')
    parser.add_argument('--checkpoint',
                        type=str,
                        default = r'G:\mmdetection_miner_result\work_dirs_rgb_depth_attention_cbam_d_simple/epoch_177.pth',
                        help='checkpoint.')
    ...
    parser.add_argument('--save-dir',
                        type=str,
                        default = r'G:\mmdetection_miner_result\work_dirs_rgb_depth_attention_cbam_d_simple\val\drise',
                        help='save dir.')
