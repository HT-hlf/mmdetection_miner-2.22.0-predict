...
@PIPELINES.register_module()
class LoadImageFromFile_rgb_depth:
    """Load an image from file.
    ...
    def __call__(self, results):
        ...
        img_depth = mmcv.imfrombytes(img_bytes_depth, flag='grayscale')
        if self.to_float32:
            img = img.astype(np.float32)
            img_depth = img_depth.astype(np.float32)
        img_depth = img_depth.reshape(*img_depth.shape, 1)
        img_rgbd = np.concatenate([img,img_depth],2)

        results['filename'] = filename
        results['ori_filename'] = results['img_info']['filename']
        results['img'] = img_rgbd
        results['img_shape'] = img_rgbd.shape
        results['ori_shape'] = img_rgbd.shape
        results['img_fields'] = ['img']
        return results
